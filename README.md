# Sonia

MSc Biotechnology | Bioinformatics & Genomics  
Passionate about multi-omics data analysis and computational biology  
Interested in disease subtyping, biomarker discovery, and translational research  

---

## About Me

- Bioinformatics enthusiast with hands-on experience in **NGS data analysis**
- Worked on **under & over-representation of CTAG motifs in bacterial genomes**
- Skilled in integrating **wet lab + computational biology**
- Interested in **multi-omics + clinical data integration**
- Goal: Pursuing a **PhD in Genomic and  Bioinformatics / Computational Biology**

---

## Research Ethics & Data Confidentiality

- Due to **data confidentiality agreements**, certain project codes and datasets cannot be shared publicly  
- Only non-sensitive, reproducible, and open-source work is available on this profile  
- Maintains strict adherence to **research ethics and data privacy standards**

---
## Tools & Technologies

### 🧬 Molecular Biology & Biotechnology

- **Genomic DNA Workflows:** Isolation, purification, and quality assessment from bacteria, human blood, chicken liver, wastewater, and soil samples  
- **Electrophoresis & Cloning:** Agarose gel electrophoresis, plasmid isolation, restriction digestion & ligation (pUC19), transformation & plating  
- **PCR & Library Prep:** PCR amplification, primer design, DNA library preparation  
- **Cell Culture:** Bacterial culture, PBMC isolation, primary human cell culture, media prep, cell counting, cryopreservation  
- **Cell-based Assays:** MTT assay, cell viability & cytotoxicity analysis  
- **Immunological Techniques:** ELISA, rocket immunoelectrophoresis  
- **Biochemical Assays:** Lowry’s assay, enzyme activity assays, titration, TLC  
- **General Lab Skills:** Buffer preparation, pipetting, microscopy, spectrophotometry  

---

### 💻 Bioinformatics & NGS Analysis

- **Sequence Analysis:** BLAST, ClustalW, primer design, Sequence Manipulation Suite  
- **Databases & Structural Tools:** NCBI, PDB, SWISS-MODEL  

- **NGS Data Processing:**  
  FastQC, MultiQC, RSeQC, Fastp, TrimGalore, Trimmomatic, NanoFilt, Porechop, FastqScreen  

- **Alignment & Variant Calling:**  
  BWA, Bowtie2, STAR, Samtools, Picard, GATK  

- **Metagenomics & Assembly:**  
  Kraken2, SPAdes, Ragout, Flye, Minimap2, Krona  

- **Workflow Management:**  
  Snakemake, Bioconductor  

---

### 🧬 Genomic Data Analysis & Clinical Interpretation

- **Variant Annotation & Databases:**  
  ClinVar, dbSNP, gnomAD, COSMIC, OMIM, Ensembl  

- **Clinical & Pharmacogenomics:**  
  PharmGKB, CPIC, OncoKB, CIViC, WHO, FDA  

---

### 📊 Data Visualization

- Circos, ComplexHeatmap, pheatmap  
- Matplotlib, Seaborn, Plotly  
- Sunburst, GeoPandas, Folium, Kepler.gl  

---

### 💻 Programming & Computational Skills

- **Operating System:** Linux  
- **Programming:** Python (Biopython), R (basic)  
- **Workflow Management:** Snakemake  

---

### Current Work

- Whole Exome Sequencing (WES)  
- Whole Genome Sequencing (WGS)  
- Targeted RNA Sequencing  
- Hybrid/Blended Exome-Genome Analysis  

---

###  IT & Documentation Tools

- MS Word, Excel, PowerPoint  
- GraphPad Prism  
- LaTeX  
- Figma   

---

##  Featured Projects

### CTAG Motif Analysis Pipeline
Study of under/over-representation of CTAG in bacterial genomes  
Tools: Python, Biopython, Linux  
Includes genome-wide and gene-level analysis  



---

### NGS Data Analysis Pipeline
End-to-end workflow for sequencing data  
Tools: FastQC, Porechop, NanoFilt, Samtools, Kraken  
Applied in metagenomics and infectious disease analysis and SARS-CoV-2 analysis  

---

## 🔬 Research Interests

- Genetic Disorders & Genomic Variation
- Bioinformatics & Computational Biology
- Genomics & Multi-omics Integration
- Disease Subtyping
- Precision Medicine 

---
## GitHub Stats

![Sonia's GitHub stats](https://github-readme-stats.vercel.app/api?username=sonia-genomics&show_icons=true&theme=default&cache_seconds=1800)

---

## Connect with Me

-  LinkedIn: https://www.linkedin.com/in/sonia-0a97341a5  
-  Email: soniakaurbio05@gmail.com 

---
⭐ *Open to collaborations, research opportunities, and PhD positions*
